            ...
            if 'attn' in perm_id:
                ...
                if all_heads_indices is None:
                    if axis == 0:
                        ...
                        param = torch.index_select(param, axis, perm.int()) #opt 1 is the same as opt2
                    elif axis == 1:
                        # Anti-permutation of heads
                        param = param.T
                        param = torch.index_select(param.reshape(num_heads, param.shape[0]//num_heads, param.shape[1]), 0, perm)
                    ...
                    if axis == 0:
                        if len(param.shape) > 1:
                            param = param.reshape(num_heads, original_shape[0]//num_heads, -1)
                            param = torch.index_select(param, axis, perm.int()) #opt 1 is the same as opt2
                            param = param.transpose(1,0)
                            for i in range(num_heads):
                                param[:,i] = torch.index_select(param[:,i], axis, heads_perm[f'P_head_{i}'].cuda())
                            param = param.transpose(1,0)
                        else:
                            param = param.reshape(num_heads, -1)
                            param = torch.index_select(param, axis, perm.int()) #opt 1 is the same as opt2
                        ...
                        if len(param.shape) > 1:
                            param = param.T
                            param = param.reshape(num_heads, param.shape[0]//num_heads, param.shape[1])
                            param = torch.index_select(param, 0, perm)
                            param = param.transpose(1,0)
                            for i in range(num_heads):
                               param[:,i] = torch.index_select(param[:,i], 0, heads_perm[f'P_head_{i}'].cuda())
                            param = param.transpose(1,0)
                            param = param.reshape(original_shape[1],original_shape[0])
                            param = param.T
                        else:
...
def cosine_similarity_models(model1_dict, model2_dict):
    ...
    for key in model1_dict:
        ...
        if key in model2_dict:
            similarities.append(
                cosine_similarity(model1_dict[key].flatten(), model2_dict[key].flatten(), dim=0)
            )
...
def apply_permutation_to_statedict(ps: PermutationSpec, perm_matrices, model_a_dict, model_b_dict=None, heads_permutation=None, skip_params=False, num_heads=12):
    ...
    for param_name, param in model_a_dict.items():
        ...
        try:
            ...
            print(f"Problem during application of permutation {perms_to_apply} on layer {param_name}")
...
def apply_permutation_to_task_vector(ps: PermutationSpec, perm_matrices, model_a_dict, model_b_dict, task_vector_dict, heads_permutation=None, skip_params=False, num_heads=12):
    ...
    permuted_params = {}
    linear_params = [param for param in model_a_dict.keys() if "fc" in param or param == 'proj']
    ...
    for param_name, param in task_vector_dict.items():
        ...
        try:
            ...
            if 'bias' in param_name or 'ln_' in param_name:
                ...
                model_a_param = get_permuted_param(model_a_dict[param_name], perms_to_apply, perm_matrices, all_heads_indices=heads_permutation, num_heads=num_heads)
                
                model_b_param = model_b_dict[param_name]
                
                cosine_sim_matrix = (model_b_param @ model_a_param.T)/(torch.norm(model_b_param, dim=1).unsqueeze(1) * torch.norm(model_a_param, dim=1).unsqueeze(0))
                cosine_sim_vector = torch.diag(cosine_sim_matrix)
                
                mask = cosine_sim_vector < threshold
                print(f"Mantained params for {param_name}: {mask.sum()} of {len(mask)}")
            ...
            print(f"{e} during application of permutation {perms_to_apply} on layer {param_name}")
            permuted_params[param_name] = permuted_params[param_name] = torch.zeros_like(param)
...
def singular_values_norm_multihead(a: torch.Tensor,b: torch.Tensor, k=10) -> torch.Tensor:
    ...
    diff_sings = svd_vals_a.unsqueeze(1) - svd_vals_b.unsqueeze(0) #differenze tra vettori di valori singolari all head_a vs all_head_b
...
def singular_value_similarity_matrix(a, b, k=10):
    ...
    svd_vals_a = torch.stack([torch.linalg.svdvals(a[i])[:k] for i in range(num_heads_a)])
    svd_vals_b = torch.stack([torch.linalg.svdvals(b[j])[:k] for j in range(num_heads_b)])
    ...
    for i in range(num_heads_a):
        for j in range(num_heads_b):
            ...
            similarity_matrix[i, j] = wasserstein_distance(svd_vals_a[i].cpu().numpy(), svd_vals_b[j].cpu().numpy())
...
def procrustes_similarity_matrix(a, b):
    ...
    for i in range(num_heads_a):
        for j in range(num_heads_b):
            ...
            similarity_matrix[i, j] = np.linalg.norm(aligned_a - matrix_b, 'fro')
...
def spectral_similarity_matrix(a, b, k=10):
    ...
    for i in range(num_heads_a):
        ...
        for j in range(num_heads_b):
            ...
            similarity_matrix[i, j] = wasserstein_distance(eigenvals_a.cpu().numpy(), eigenvals_b.cpu().numpy())
...
def bures_wasserstein_similarity_matrix(a, b):
    ...
    for i in range(num_heads_a):
        ...
        for j in range(num_heads_b):
            ...
            sqrt_cov_a = torch.linalg.cholesky(cov_a + 1e-6 * torch.eye(cov_a.size(0)))  # Adding a small epsilon for stability
            
            # Compute the intermediate term for Bures-Wasserstein distance
            product = sqrt_cov_a @ cov_b @ sqrt_cov_a
            sqrt_product = torch.linalg.cholesky(product + 1e-6 * torch.eye(product.size(0)))  # Square root of the product matrix
            
            # Compute the Bures-Wasserstein distance
            similarity_matrix[i, j] = torch.trace(cov_a + cov_b - 2 * sqrt_product)
...
def cosine_similarity_matrix(a, b):
    ...
    for i in range(num_heads_a):
        for j in range(num_heads_b):
            ...
            similarity_matrix[i, j] = F.cosine_similarity(head_a_flat, head_b_flat, dim=0)
...
def cca_similarity_matrix(a, b, num_components=5):
    ...
    for i in range(num_heads_a):
        for j in range(num_heads_b):
            ...
            correlations = [torch.corrcoef(torch.stack([top_u_a[:, k], top_u_b[:, k]]))[0, 1] for k in range(num_components)]
...
def cca(activation1, activation2):
    ...
    covariance_11 = torch.matmul(activation1.T, activation1) / (activation1.shape[0] - 1)
    covariance_22 = torch.matmul(activation2.T, activation2) / (activation2.shape[0] - 1)
    covariance_12 = torch.matmul(activation1.T, activation2) / (activation1.shape[0] - 1)
    ...
    mat_a = torch.matmul(torch.matmul(inv_cov_11, covariance_12), torch.matmul(inv_cov_22, covariance_12.T))
    mat_b = torch.matmul(torch.matmul(inv_cov_22, covariance_12.T), torch.matmul(inv_cov_11, covariance_12))
...
def batched_emd_similarity_matrix(A, B, batch_size=100):
    ...
    for i in range(0, A_rows, batch_size):
        ...
        for j in range(0, B_rows, batch_size):
            ...
            A_expanded = A_batch.unsqueeze(1)  # Shape: (batch_size, 1, features)
            B_expanded = B_batch.unsqueeze(0)  # Shape: (1, batch_size, features)
            emd_batch = earth_movers_distance(A_expanded, B_expanded)  # Shape: (batch_size, batch_size)
            similarity_batch = 1 / (1 + emd_batch)
            
            # Store the result in the full similarity matrix
            similarity_matrix[i:i+batch_size, j:j+batch_size] = similarity_batch
