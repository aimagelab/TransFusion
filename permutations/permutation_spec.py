    ...
    def create_permutation_spec(self) -> PermutationSpec:
        ...
        axes_to_perm = {
            **conv_axes(f"{prefix}conv1", in_perm=None, out_perm="P_conv"),
            ...
            f"{prefix}positional_embedding": (None, "P_conv"),  # (1, 1, dim) probably P_transf_in or its own P
            f"{prefix}ln_pre.weight": ("P_conv",),  # (1, 1, dim) probably P_transf_in or its own P
            f"{prefix}ln_pre.bias": ("P_conv",),  # (1, 1, dim) probably P_transf_in or its own P
            ...
            **transformer_block_axes_clip(self.depth, p_in="P_conv", p_out='P_last', prefix=prefix),
            ...
            f"{prefix}proj": ('P_last', None ),  # (1, 1, dim) probably P_transf_in or its own P
...
def transformer_block_axes_clip(depth, p_in, p_out, prefix=''):
    ...
    for block_ind in range(depth):
        ...
        block_axes = {
            # Attention
            ## layer norm 1
            f"{prefix}transformer.resblocks.{block_ind}.ln_1.weight": (f"P{block_ind}_ln1", block_in),  # (dim,)
            f"{prefix}transformer.resblocks.{block_ind}.ln_1.bias": (f"P{block_ind}_ln1",),  # (dim,)
            # HEADS
            f"{prefix}transformer.resblocks.{block_ind}.attn.q.weight": (f"P{block_ind}_attn_QK", f"P{block_ind}_ln1"),  # (head_dim, dim) f"P{block_ind}_attn_QK_"
            f"{prefix}transformer.resblocks.{block_ind}.attn.k.weight": (f"P{block_ind}_attn_QK", f"P{block_ind}_ln1"),  # (head_dim, dim)
            f"{prefix}transformer.resblocks.{block_ind}.attn.v.weight": (f"P{block_ind}_attn_QK", f"P{block_ind}_ln1"),  # (head_dim, dim)
            f"{prefix}transformer.resblocks.{block_ind}.attn.q.bias": (f"P{block_ind}_attn_QK",),  # ( dim) ?
            f"{prefix}transformer.resblocks.{block_ind}.attn.k.bias": (f"P{block_ind}_attn_QK",),  # (dim) ?
            f"{prefix}transformer.resblocks.{block_ind}.attn.v.bias": (f"P{block_ind}_attn_QK",),  # (dim) ?

            # Attention out
           
            f"{prefix}transformer.resblocks.{block_ind}.attn.proj.weight":(f"P{block_ind}_out_proj", f"P{block_ind}_attn_QK"),
            f"transformer.resblocks.{block_ind}.attn.proj.bias": (f"P{block_ind}_out_proj",), #(f"P_{block_ind}_out_proj",),
            # shortcut
            f"transformer.resblocks.{block_ind}.attn.shortcut_1.identity": (f"P{block_ind}_out_proj", block_in),  # (dim, dim) # WORKS
            # MLP
            ## layer norm 2
            f"{prefix}transformer.resblocks.{block_ind}.ln_2.weight": (f"P{block_ind}_ln2", f"P{block_ind}_out_proj",),  # (dim,)
            f"{prefix}transformer.resblocks.{block_ind}.ln_2.bias": (f"P{block_ind}_ln2", ),  # (dim,)
            ...
            f"{prefix}transformer.resblocks.{block_ind}.mlp.fc1.bias": (f"P{block_ind}_mlp_out",),  # (mlp_dim,)
            ## linear 2
            f"{prefix}transformer.resblocks.{block_ind}.mlp.fc2.weight":(block_out, f"P{block_ind}_mlp_out",), #(block_out,f"P{block_ind}_mlp_out",),  # (dim, mlp_dim) # WORKS
            f"{prefix}transformer.resblocks.{block_ind}.mlp.fc2.bias": (block_out,), #(block_out,),  # (dim,) # WORKS
            # shortcut 2
             f"transformer.resblocks.{block_ind}.mlp.shortcut_2.identity": (block_out, f"P{block_ind}_out_proj"),  # (dim, dim) # WORKS
        }
...
def transformer_block_axes(depth, p_in, p_out):
    ...
    for block_ind in range(depth):
        ...
        block_axes = {
            ...
            f"transformer.layers.{block_ind}.0.to_v.weight": (f"P{block_ind}_attn_QK", block_in),  # (head_dim, dim)
            # Attention out
            f"transformer.layers.{block_ind}.0.to_out.0.weight": (f"P{block_ind}_out_proj" , f"P{block_ind}_attn_QK"),  # (dim, dim)
            f"transformer.layers.{block_ind}.0.to_out.0.bias": (f"P{block_ind}_out_proj",),  # (dim,)
            # shortcut
            f"transformer.layers.{block_ind}.1.identity": (f"P{block_ind}_out_proj", block_in),  # (dim, dim) # WORKS
            # MLP
            ## layer norm
            f"transformer.layers.{block_ind}.2.net.0.weight": (f"P{block_ind}_out_proj",),  # (dim, )
            f"transformer.layers.{block_ind}.2.net.0.bias": (f"P{block_ind}_out_proj",),  # (dim,)
            ## linear 1
            f"transformer.layers.{block_ind}.2.net.1.weight": (f"P{block_ind}_mlp_out", f"P{block_ind}_out_proj",),  # (mlp_dim, dim)
            f"transformer.layers.{block_ind}.2.net.1.bias": (f"P{block_ind}_mlp_out",),  # (mlp_dim,)
            ## linear 2
            f"transformer.layers.{block_ind}.2.net.4.weight": (block_out, f"P{block_ind}_mlp_out",),  # (dim, mlp_dim) # WORKS
            f"transformer.layers.{block_ind}.2.net.4.bias": (block_out,),  # (dim,) # WORKS
            # shortcut 2
            f"transformer.layers.{block_ind}.3.identity": (block_out, f"P{block_ind}_out_proj"),  # (dim, dim) # WORKS
        }
...
class ViT_B_PermutationSpecBuilder(PermutationSpecBuilder):
    ...
    def create_permutation_spec(self, **kwargs) -> PermutationSpec:
        axes_to_perm = {
            ...
            "vit.embeddings.position_embeddings": (None, None, "P_in"),  # (1, num_patches+1, dim)
            "vit.embeddings.patch_embeddings.projection.weight": ("P_in",),  # (dim, channels, kernel_h, kernel_w)
            "vit.embeddings.patch_embeddings.projection.bias": ("P_in",),  # (dim)
    ...
    def vit_b_transformer_block_axes(self, depth, p_in, p_out):
        ...
        for layer_idx in range(depth):
            ...
            block_out = p_out if layer_idx == depth - 1 else f"P{layer_idx}_out"

            block_axes = {
                
                # Layer Norm before attention
                f"vit.encoder.layer.{layer_idx}.layernorm_before.weight": (block_in,),  # (dim)
                f"vit.encoder.layer.{layer_idx}.layernorm_before.bias": (block_in,),  # (dim)
                
                # Attention: Query, Key, Value
                f"vit.encoder.layer.{layer_idx}.attention.attention.query.weight": (f"P{layer_idx}_attn", block_in),  # (head_dim, dim)
                f"vit.encoder.layer.{layer_idx}.attention.attention.query.bias": (f"P{layer_idx}_attn",),  # (head_dim)
                f"vit.encoder.layer.{layer_idx}.attention.attention.key.weight": (f"P{layer_idx}_attn", block_in),  # (head_dim, dim)
                f"vit.encoder.layer.{layer_idx}.attention.attention.key.bias": (f"P{layer_idx}_attn",),  # (head_dim)
                f"vit.encoder.layer.{layer_idx}.attention.attention.value.weight": (f"P{layer_idx}_attn", block_in),  # (head_dim, dim)
                f"vit.encoder.layer.{layer_idx}.attention.attention.value.bias": (f"P{layer_idx}_attn",),  # (head_dim)

                # Attention output
                f"vit.encoder.layer.{layer_idx}.attention.output.dense.weight": (f"P{layer_idx}_dense_out", f"P{layer_idx}_attn"),  # (dim, head_dim)
                f"vit.encoder.layer.{layer_idx}.attention.output.dense.bias": (f"P{layer_idx}_dense_out",),  # (dim)
                
                # Attention output shortcut
                f"vit.encoder.layer.{layer_idx}.attention.shortcut_1.identity": (f"P{layer_idx}_dense_out", block_in),  # (dim, dim)
                
                # Layer Norm after attention and residual
                f"vit.encoder.layer.{layer_idx}.layernorm_after.weight": (f"P{layer_idx}_dense_out",),  # (dim)
                f"vit.encoder.layer.{layer_idx}.layernorm_after.bias": (f"P{layer_idx}_dense_out",),  # (dim)

                # MLP Layers
                f"vit.encoder.layer.{layer_idx}.intermediate.dense.weight": (f"P{layer_idx}_mlp", f"P{layer_idx}_dense_out"),  # (mlp_dim, dim)
                f"vit.encoder.layer.{layer_idx}.intermediate.dense.bias": (f"P{layer_idx}_mlp",),  # (mlp_dim)
                f"vit.encoder.layer.{layer_idx}.output.dense.weight": (block_out, f"P{layer_idx}_mlp"),  # (dim, mlp_dim)
                f"vit.encoder.layer.{layer_idx}.output.dense.bias": (block_out,),  # (dim)
                
                # MLP output shortcut
                f"vit.encoder.layer.{layer_idx}.attention.shortcut_2.identity": (block_out, f"P{layer_idx}_dense_out"),  # (dim, dim)
    ...
    def naive_vit_b_transformer_block_axes(self, depth, p_in, p_out):
        ...
        for layer_idx in range(depth):
            ...
            block_axes = {
                ...
                # Attention output shortcut
                f"vit.encoder.layer.{layer_idx}.attention.shortcut_1.identity": (None, None),  # (dim, dim)
                ...
                # MLP output shortcut
                f"vit.encoder.layer.{layer_idx}.attention.shortcut_2.identity": (None, None),  # (dim, dim)
