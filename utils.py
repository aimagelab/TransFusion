...
def load_dataset(args, preprocess, support=False, few_shot=False):
    if support:
        ...
        if args.dataset == 'cifar100':   
            target_dataset = CIFAR100(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'eurosat':
            target_dataset = EuroSat(root=f"{args.base_folder}/datasets/eurosat", split='test', transform=preprocess) ############
            target_dataloader = DataLoader(target_dataset, batch_size=args.batch_size, shuffle=False,
                num_workers=args.workers, pin_memory=True, drop_last=False)
        elif args.dataset == 'sun397':
            target_dataset = SUN397(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'cars':
            target_dataset = Cars(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'dtd':
            target_dataset = DTD(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'svhn':
            target_dataset = SVHN(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'gtsrb':
            target_dataset = GTSRB(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'resisc45':
            target_dataset = RESISC45(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            target_dataloader = target_dataset.test_loader
        elif args.dataset == 'imagenetr':
            target_dataset = IMAGENETR(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers)
            target_dataloader = target_dataset.test_loader
        else:
        ...
        if args.dataset == 'cifar100': 
            dataset = CIFAR100(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            ...
            dataset = EuroSat(root=f'{args.base_folder}/datasets/eurosat', split='train', transform=preprocess)
            if few_shot:
            ...
            dataset = SUN397(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            test_loader = dataset.test_loader
            train_loader = dataset.train_loader
        elif args.dataset == 'cars':
            dataset = Cars(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            test_loader = dataset.test_loader
            train_loader = dataset.train_loader
        elif args.dataset == 'dtd':
            dataset = DTD(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            test_loader = dataset.test_loader
            train_loader = dataset.train_loader
        elif args.dataset == 'svhn':
            dataset = SVHN(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            test_loader = dataset.test_loader
            train_loader = dataset.train_loader
        elif args.dataset == 'gtsrb':
            dataset = GTSRB(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            test_loader = dataset.test_loader
            train_loader = dataset.train_loader
        elif args.dataset == 'resisc45':
            dataset = RESISC45(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers, batch_size=args.batch_size)
            test_loader = dataset.test_loader
            train_loader = dataset.train_loader
        elif args.dataset == 'imagenetr':
            dataset = IMAGENETR(preprocess=preprocess, location=f'{args.base_folder}/datasets', num_workers=args.workers)
            test_loader = dataset.test_loader
            train_loader = None
        else:
            raise ValueError(f"Invalid dataset: {args.dataset}")
...
def evaluate_model(model, dataloader, dataset, device='cuda:0', prompt_ensemble=True, first_n_batches=None):
    ...
    for id, batch in tqdm(enumerate(dataloader)) :
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            vl_logits = 100 * (torch.einsum('ij,cj->ic',image_features, text_features))
