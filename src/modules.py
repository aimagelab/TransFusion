...
def count_number_of_groups_csv(csv_summary):
    df = pd.read_csv(csv_summary)
    groups = {}
    for split in ['training', 'test']:
        df_ = df.loc[df['use'] == split]
        race = df_.groupby(['race'])['filename'].count().to_list()
        gender = df_.groupby(['gender'])['filename'].count().to_list()
        ethnicity = df_.groupby(['ethnicity'])['filename'].count().to_list()
        ethnicity[0], ethnicity[1] = ethnicity[1], ethnicity[0] #swap fix in order to be consistent with ethnicity mapping
        groups[split] = [race, gender, ethnicity]
    return groups
def get_stratified_labels_glc(csv_summary="FairCLIP/data_summary_glaucoma.csv"):
    df = pd.read_csv(csv_summary)
    glc_map = {'yes' : 1, 'no' : 0}
    glc_labels = df["glaucoma"].map(glc_map).to_numpy()
    return glc_labels

def get_stratified_labels_utk(files, dataset_label):
...
def get_instances(file):
    data = np.load(file)
    return (data['race'].item(), data['gender'].item(), data['ethnicity'].item())
...
def count_number_of_groups(input_dataset):
    ...
    with Pool(num_cpus) as p:
        rge_list = p.map(get_instances, npz_paths)
    for race, gender, ethnicity in rge_list:
        instances_on_race.append(race)
        instances_on_gender.append(gender)
        instances_on_ethnicity.append(ethnicity)
...
def count_number_of_groups_utk(input_dataset):
    instances_on_race = []
    instances_on_gender = []
    instances_on_age = []
    npz_paths = []
    for file in input_dataset.files:
        ...
        instances_on_race.append(race)
        instances_on_gender.append(gender)
        instances_on_age.append(age)
        
    _, numbers_of_race = np.unique(instances_on_race, return_counts=True)
    _, numbers_of_gender = np.unique(instances_on_gender, return_counts=True)
    _, numbers_of_age = np.unique(instances_on_age, return_counts=True)
    return numbers_of_race, numbers_of_gender, numbers_of_age

class fair_vl_med_dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir='', preprocess=None, files=None, subset='Training', text_source='note', summarized_note_file=None, ruleout_unknown=False, fold_idx=None):
        ...
        if files is not None:
            ...
            self.files = sorted(find_all_files(self.dataset_dir, suffix='npz'))
            #select only files in fold_idx list   
            if fold_idx:
                self.files = [self.files[i] for i in fold_idx]
...
def compute_vl_prob(img_feats, class_txt_feats):
    ...
    class_txt_feats = class_txt_feats.permute(1,2,0)
    similarity_per_class = (img_feats @ class_txt_feats)
    all_logits = torch.diagonal(similarity_per_class, dim1=1, dim2=2)
...
def new_accuracy(outputs, targets):
    with torch.no_grad():
        batch_size = targets.shape[0]
        _, preds = torch.from_numpy(outputs).topk(1, dim=1)
        targets = torch.from_numpy(targets).view(batch_size, 1)
...
def compute_auc(pred_prob, y, num_classes=2):
    ...
    if num_classes == 2:
        ...
        auc_val = roc_auc_score(y, pred_prob, average='macro', multi_class='ovr')
...
def equity_scaled_AUC(output, target, attrs, alpha=1., num_classes=2):
    ...
    if num_classes == 2:
        ...
        overall_auc = roc_auc_score(target, output, average='macro', multi_class='ovr')

    for one_attr in np.unique(attrs).astype(int):
        pred_group = output[attrs == one_attr]
        gt_group = target[attrs == one_attr]

        # _, pred_group = torch.from_numpy(pred_group).topk(1, dim =1) #from probability of each class to class that has the higher prob
        if num_classes == 2:
            fpr, tpr, thresholds = roc_curve(gt_group, pred_group)
            group_auc = auc(fpr, tpr)
        elif num_classes > 2:
            # y_onehot = num_to_onehot(gt_group, num_classes)
            group_auc = roc_auc_score(gt_group, pred_group, average='macro', multi_class='ovr')
...
def structured_metrics(metrics : list):
  attributes_names = {'Race' : ['Asian', 'Black', 'White'], 'Gender' : ['Female', 'Male'], 'Ethnicity' : ['Non-Hispanic', 'Hispanic'], 'Language' : ['English', 'Spanish', 'Others']}
  for i,metric in enumerate(metrics):
    if any(isinstance(i, list) for i in metric):
        metrics[i] = {key : {attr : val for attr, val in zip(values, metric[i])} for i, (key,values) in enumerate(attributes_names.items())}
    else:
        metrics[i] = {attr : val for attr, val in zip(attributes_names.keys(), metric)}
  return metrics
...
def evaluate_comprehensive_perf(preds, gts, attrs=None, num_classes=2):
    ...
    # overall_acc = new_accuracy(preds, gts)
    ...
    metrics = [esaccs_by_attrs, esaucs_by_attrs, aucs_by_attrs, dpds, eods]
    labeled_metrics = structured_metrics(metrics)
    esaccs_by_attrs, esaucs_by_attrs, aucs_by_attrs, dpds, eods = labeled_metrics
    between_group_disparity = {attr : {t : v for t,v in zip(['std', 'max-min'], values)} for attr, values in zip(['Race', 'Gender', 'Ethnicity', 'Language'], between_group_disparity)}
