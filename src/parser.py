...
def add_experiment_args(parser: ArgumentParser) -> None:
    ...
    exp_group.add_argument('--eval_future', type=int, choices=(0, 1), default=0, help='Evaluate future tasks? Requires a model that extends `FutureModel`')
    exp_group.add_argument('--support_dataset', type=custom_str_underscore, choices=get_dataset_names(names_only=True),
                           help='Name of a support dataset to evaluate on, after each task. Requires a model that extends `FutureModel`')
    ...
    opt_group.add_argument('--optim_adam_betas', type=float, default=[0.9, 0.999], nargs='+',
                           help='beta parameters, used if `--optimizer=adam*`.')
    ...
    rebasin_group = parser.add_argument_group('Re-Basing related args', 'Arguments used to defined the re-basing strategy and checkpoints.')

    rebasin_group.add_argument('--rebasin_checkpoint', type=str, default=None,
                               help='Path of the checkpoint to load for re-basing.')
    rebasin_group.add_argument('--rebasin_strategy', type=str, default='soup', choices=['soup', 'swap'],
                               help='Re-basing strategy. Possible values:'
                               ' - `soup`: average the weights of the previous checkpoint and the new one.'
                               ' - `swap`: swap the weights of the previous checkpoint with the new one.')
    rebasin_group.add_argument('--rebasin_alpha', type=float,
                               help='Alpha parameter used in the re-basing strategy. Used as `alpha*A + (1-alpha)*B.')
    rebasin_group.add_argument('--rebasin_lambda', type=float,
                               help='Lambda parameter used in the re-basing strategy. Used as `A + lambda*tau.')
    rebasin_group.add_argument('--rebase_layers', type=str, default=['clip_model'], nargs='+',
                               help='Layers to re-base. If not provided, all the layers are re-based. '
                               'FORMAT: `--rebase_layers layer1 layer2 ...` will apply re-basing to all the layers that contain '
                               '`layer1`, `layer2`, etc. in their name. '
                               'EXAMPLE: `--rebase_layers conv attn` will apply re-basing '
                               'to all the layers that contain `conv` or `attn` in their name.')
    rebasin_group.add_argument('--rebase_after', type=int,
                               help='Applies re-basing only once, AFTER task.')
    rebasin_group.add_argument('--rebase_every', type=int,
                               help='Re-base every `rebasin_every` tasks.')
    rebasin_group.add_argument('--rebase_exclude_batchnorm', type=binary_to_boolean_type, default=0,
                               help='Exclude BatchNorm layers from re-basing?')
    rebasin_group.add_argument('--rebasin_task_vector_scaling_alpha', type=float, default=1,
                               help='Scaling factor applied to the task vector before applying it to the model, after rebasing.')
