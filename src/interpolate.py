...
def linear_interpolation(pretrained=None, finetuned=None, gamma=0.5):
    ...
    with torch.no_grad():
        ...
        assert pretrained_state_dict.keys() == finetuned_state_dict.keys(), "Models have different parameter keys"
        ...
        for key in pretrained_state_dict:
            ...
            interpolated_param = gamma * param_finetuned + (1 - gamma) * param_pretrained
...
def orthogonal_interpolation(pretrained=None, finetuned=None, gamma=0.5, beta=5.0):
    ...
    with torch.no_grad():
        ...
        orthogonality_measures = compute_orthogonality_measure(pretrained_state_dict, finetuned_state_dict)
        ...
        for key in pretrained_state_dict:
            ...
            if ('ln_' in key):
                ...
                adjusted_gamma = adjust_gamma_based_on_orthogonality(gamma, cos_theta, beta)
                ...
                same_sign_mask = (sign_pretrained == sign_finetuned) & (sign_pretrained != 0) & (sign_finetuned != 0)

                interpolated_param = torch.where(
                    same_sign_mask,
                    adjusted_gamma * param_finetuned + (1 - adjusted_gamma) * param_pretrained,
                    param_pretrained
                )
                model[key] = interpolated_param
